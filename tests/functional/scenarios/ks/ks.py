...
class KSTest(EquationTestTemplate):
    ...
    def correct_symbolic(self):
        return "-1.0 * d^2u/dx1^2{power: 1.0} + -1.0 * d^4u/dx1^4{power: 1.0} + -1.0 * u{power: 1.0} * du/dx1{power: 1.0} + 0.0 = du/dx0{power: 1.0}"

    def incorrect_symbolic(self):
        return "-1.0 * d^2u/dx1^2{power: 1.0} + 1.5 * u{power: 1.0} + -0.0 = du/dx0{power: 1.0}"

    @staticmethod
    def noise_data(data, noise_level):
        return noise_level * 0.01 * np.std(data) * np.random.normal(size=data.shape) + data

    def load_ks_data(self, filename):
    ...
    def make_report_dir(self, base_dir, operator_name):
        ...
        report_dir = base_dir / "ks" / operator_name / stamp
    ...
    def run_discovery(self, search_obj, report_dir=None, operator_name="unknown"):
        ...
        search_obj.set_moeadd_params(population_size=16, training_epochs=3)
