...
def autocatalysis_gs_discovery(noise_level):
    data_file = os.path.join(os.path.dirname(__file__), 'autocatalytic-gene-switching.npz')
    ...
    epde_search_obj.fit(data=[u], variable_names=['u'], max_deriv_order=(3,),
...
def autocatalysis_discovery(noise_level):
    data_file = os.path.join(os.path.dirname(__file__), 'autocatalysis.npz')
    data = np.load(data_file)
    t = data['t']
    u = data['u']
...
if __name__ == "__main__":
    ...
    # autocatalysis_discovery(0)
    autocatalysis_gs_discovery(0)
